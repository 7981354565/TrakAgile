...
const AdminDashboard = () => {
 ...
 useEffect(() => {
  ...
  const handleLocationSearch = async () => {
    ...
    try {
      const response = await fetch(`https://nominatim.openstreetmap.org/search?q=${locationSearch}&format=json&limit=1`);
      const data = await response.json();
  
      if (data && data.length > 0) {
        const { lat, lon, display_name } = data[0]; // Get first result and location name
        setSearchLocationCoords({ lat: parseFloat(lat), lon: parseFloat(lon) });
        setSearchLocationName(display_name); // Set the location name to searchLocationName
      } else {
  ...
  return (
    <div className={`dashboard ${isFullScreen ? 'full-screen' : ''}`}>
      ...
      <div className="map">
       <MapComponent  selectedPackage={selectedPackage}  searchLocationCoords={searchLocationCoords}   searchLocationName={searchLocationName} />
