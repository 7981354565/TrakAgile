...
import { Form, Button, Table, Row, Col, Tabs, Tab, Modal,Card, Spinner } from 'react-bootstrap';
...
function TaskUpdateForm({ onClose }) {
  ...
  const fetchNotesHistory = async () => {
    try {
      const response = await axios.get(`http://68.183.86.1:8080/trackagile/notes/getNotes/${task.id}`);
  ...
  const handleSaveNote = async () => {
    ...
    try {
      console.log(task.id);
      await axios.post('http://68.183.86.1:8080/trackagile/notes/save', {
        id: task.id, // Pass task ID here
        note: note  // Pass the note content here
      });
  
      alert('Task note created successfully!'); // Success alert
      setNote(''); // Clear the input field after saving
      fetchNotesHistory(); // Refresh the notes history
    } catch (error) {
      console.error('Error saving note:', error);
      alert('Failed to create task note. Please try again.'); // Error alert
    }
  };
  ...
  const fetchHistoryData = async () => {
    try {
      const response = await fetch(`http://68.183.86.1:8080/trackagile/history/get/${task.id}`);
  ...
  return (
                ...
                <Col md={6}>
                  ...
                  <Form.Group controlId="formAssignedTo">
                      ...
                      <Form.Control
                          type="text"
                          name="assignedTo"
                          value={finalEmployee?.empName || formData.assignedTo} // Display confirmed employee's name
    ...
    {key === 'history' && (
      <div className="history-section">
        {loading ? (
          <Spinner animation="border" /> // Show a loading spinner while fetching data
        ) : (
          <ul>
            {historyData.map((item, index) => (
              <Card key={index} className="mb-3">
                <Card.Body>
                  <Card.Title>Action: {item.actCode}</Card.Title>
                  <Card.Text>
                    <strong>Description:</strong> {item.description} <br />
                    <strong>Performed By:</strong> {item.performedBy || 'N/A'} <br />
                    <strong>Time:</strong> {item.actTime || 'N/A'}
                  </Card.Text>
                </Card.Body>
              </Card>
            ))}
        ...
        <div className="notes-history">
          ...
          {notesHistory.length > 0 ? (
            <div className="notes-cards-container">
              {notesHistory.map((noteItem) => (
                <div key={noteItem.id} className="note-card">
                  <p>
                    <strong>Logged By</strong>
                    <span className="colon">:</span>
                    <span>{noteItem.loggedBy}</span>
                  </p>
                  <p>
                    <strong>Time</strong>
                    <span className="colon">:</span>
                    <span>{noteItem.loggedTime}</span>
                  </p>
                  <p>
                    <strong>Note</strong>
                    <span className="colon">:</span>
